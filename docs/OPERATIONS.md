...
- **Two private links, each behind its own access key.** Keys are ~100 bits,
  stored only as scrypt hashes in `.env`. A wrong link gives a plain 404; the bare domain shows an access page: entering either key there opens the matching area (shareholder report or admin), with its own wrong-key lockout.
